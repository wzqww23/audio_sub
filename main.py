...
version = "0.20.1"
print(
    f"------- 'Auto Synced Translated Dubs' script by ThioJoe - Release version {version} -------"
)
...
for num in languageNums:
    ...
    if not batchConfig.has_section(f"LANGUAGE-{num}"):
        raise ValueError(
            f"Invalid language number in batch.ini: {num} - Make sure the section [LANGUAGE-{num}] exists"
        )
...
for num in languageNums:
    if not batchConfig.has_option(f"LANGUAGE-{num}", "synth_language_code"):
        raise ValueError(
            f'Invalid configuration in batch.ini: {num} - Make sure the option "synth_language_code" exists under [LANGUAGE-{num}]'
        )
    if not batchConfig.has_option(f"LANGUAGE-{num}", "synth_voice_name"):
        raise ValueError(
            f'Invalid configuration in batch.ini: {num} - Make sure the option "synth_voice_name" exists under [LANGUAGE-{num}]'
        )
    if not batchConfig.has_option(f"LANGUAGE-{num}", "translation_target_language"):
        raise ValueError(
            f'Invalid configuration in batch.ini: {num} - Make sure the option "translation_target_language" exists under [LANGUAGE-{num}]'
        )
    if not batchConfig.has_option(f"LANGUAGE-{num}", "synth_voice_gender"):
        raise ValueError(
            f'Invalid configuration in batch.ini: {num} - Make sure the option "synth_voice_gender" exists under [LANGUAGE-{num}]'
        )
...
for num in languageNums:
    ...
    batchSettings[num] = {
        ...
        "translation_target_language": batchConfig[f"LANGUAGE-{num}"][
    ...
    for lineNum, line in enumerate(srtFileLines):
        ...
        if line.isdigit() and subtitleTimeLineRegex.match(srtFileLines[lineNum + 1]):
            ...
            while True:
                ...
                if (lineNum + count) < len(srtFileLines) and srtFileLines[
                    lineNum + count
                ].strip():
...
def get_duration(filename):
    ...
    if filename.lower().endswith(".mp3"):
        stream_selector = "a:0"  # Select the first audio stream
    else:
        stream_selector = "v:0"  # Select the first video stream for non-audio files
    result = subprocess.check_output(
        f'ffprobe -v quiet -show_streams -select_streams {stream_selector} -of json "{filename}"',
        shell=True,
    ).decode()
...
if config["debug_mode"] and ORIGINAL_VIDEO_PATH.lower() == "debug.test":
    ...
    totalAudioLength = int(
        originalLanguageSubsDict[str(len(originalLanguageSubsDict))]["end_ms"]
    )
else:
...
def get_pretranslated_subs_dict(langData):
    ...
    if altFiles and files:
        ...
        userResponse = input(
            "Use YouTube-synced translations instead of those in main output folder? (y/n): "
        )
    ...
    for file in files:
        if file.replace(" ", "").endswith(
            f"-{langData['translation_target_language']}.srt"
        ):
            ...
            preTranslatedDict = parse_srt_file(
                pretranslatedSubLines, preTranslated=True
            )
...
def process_language(langData, processedCount, totalLanguages):
    ...
    if langDict["languageCode"].lower() == config["original_language"].lower():
        ...
        individualLanguageSubsDict = translate.translate_dictionary(
            individualLanguageSubsDict,
            langDict,
        ...
        individualLanguageSubsDict = translate.translate_dictionary(
            individualLanguageSubsDict,
            langDict,
            skipTranslation=config["skip_translation"],
        )
        if config["stop_after_translation"]:
            print(
                "Stopping at translation is enabled. Skipping TTS and building audio."
            )
            return
        ...
        if pretranslatedSubsDict != None:
            ...
            print(
                f"Note: Ensure the subtitle filename for this language ends with: ' - {langData['translation_target_language']}.srt'\n"
            )
            return

    # Synthesize
    if (
        cloudConfig["batch_tts_synthesize"] == True
        and cloudConfig["tts_service"] == "azure"
    ):
        individualLanguageSubsDict = TTS.synthesize_dictionary_batch(
            individualLanguageSubsDict,
            langDict,
        ...
        individualLanguageSubsDict = asyncio.run(
            TTS.synthesize_dictionary_async(
                individualLanguageSubsDict,
                langDict,
                skipSynthesize=config["skip_synthesize"],
                max_concurrent_jobs=cloudConfig["elevenlabs_max_concurrent"],
            )
        )
    else:
        individualLanguageSubsDict = TTS.synthesize_dictionary(
            individualLanguageSubsDict,
            langDict,
            skipSynthesize=config["skip_synthesize"],
        )
    ...
    # Build audio
    individualLanguageSubsDict = audio_builder.build_audio(
        individualLanguageSubsDict,
        langDict,
        totalAudioLength,
        config["two_pass_voice_synth"],
    )
